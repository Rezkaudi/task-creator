...
function getTrelloConfig(selectedListId: string): TrelloConfig {
    const config = {
...
export async function createTrelloCards(tasks: Task[], selectedListId: string): Promise<TrelloCard[]> {
    const config = getTrelloConfig(selectedListId);
...
export async function getTrelloLists(boardId?: string, selectedListId?: string): Promise<any[]> {
    const config = getTrelloConfig(selectedListId!);
